...
- It is tagged with the exact license the source/partner actually publishes under — `CC-BY-NC-4.0` for a direct partner agreement, or whatever the original author chose for an already-published existing dataset (`CC-BY-4.0`, `CC-BY-NC-SA-4.0`, `Apache-2.0`, etc. have all been encountered in practice — see [`docs/cla-data-schema.md`](cla-data-schema.md)). The exact license string must never be assumed or normalized away and must be confirmed per source before ingestion. A `ShareAlike` clause (`*-SA-*`) carries its own obligations for anything derived from the content — flag this explicitly rather than treating all CC variants as interchangeable.
